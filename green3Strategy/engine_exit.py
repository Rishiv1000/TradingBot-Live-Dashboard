...
class ExitEngine:
    ...
    def _close_position_and_log(self, row, sell_price, reason, sell_order_id, ltp_at_sell=None):
        ...
        cursor.execute(
            ...
            (row["symbol"], row["buytime"], row["buyprice"], datetime.now(),
             ...
             str(row["buy_order_id"]), str(sell_order_id), st_config_green3.STRATEGY_NAME),
        )
        cursor.execute(
        ...
        print(f"[GREEN3] SOLD {row['symbol']} @ {sell_price} ({reason}) | PnL: ₹{round(pnl, 2)}")
    ...
    def start_monitoring(self):
        kws = KiteTicker(st_config_green3.API_KEY, self.kite.access_token)
